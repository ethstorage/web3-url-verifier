...
本项目实现了一个 web3:// 数据验证原型，模拟浏览器插件的工作模式：正常获取 web3:// 数据 → 独立验证数据真实性 → 本地输出验证结果。项目覆盖 Ethereum 状态数据和 EthStorage 文件数据两类场景，并在真实部署站点上进行了测试。
...
适用于 `eth_call`、`render()`、`tokenURI()`、`balanceOf()` 等所有合约调用。
...
**Colibri 的作用**：Colibri 的远程 Prover 返回调用结果、状态证明和区块头信息。并在 Colibri 客户端内部验证区块头、状态根和调用证明，验证通过后 `verifiedResult` 即等于链上真实结果。

### 3.2 EthStorage 文件数据

适用于 HTML、CSS、JS、PNG、Video、Model 等任意 Blob 文件。
...
FlatDirectory 合约记录 Versioned Hash，文件内容由 EthStorage 网络持久化保存。
...
| Safe 前端 | Sepolia | HTML + Next.js 静态资源 | 4.4MB（14 文件） | EthStorage 文件验证 |
| VBlog | Sepolia | HTML + CSS + 大体积 JS | 1.7MB（3 文件） | EthStorage 文件验证 |
...
───────────────────────────────────────────────────────────────────────────
 EthStorage 官网             5    1.3MB        6.4s       4.3s      0.67x
 Safe 前端                  14    4.4MB       15.5s      20.0s      1.29x
 VBlog                       3    1.7MB       10.3s       4.3s      0.42x
 NFT Render                  1   14.6KB        1.3s       7.9s      6.08x
═══════════════════════════════════════════════════════════════════════════
```

> **关于 Prover 缓存**：Colibri Prover 服务器端对合约状态有缓存机制。首次请求某个状态时 Prover 需要从链上获取并生成证明，耗时较长；后续请求命中缓存则更快。上表数据基于多次运行取平均，偏向缓存命中后的典型性能。由于 Prover 是共享公共服务，无法精确控制冷启动条件，冷缓存下实际耗时可能略高于表中数值。
...
| Safe 前端 | 14 | 13 | 1 | 根文件：数据获取路径注入额外脚本 |
| VBlog | 3 | 3 | 0 | 全部通过 |
| NFT Render | 1 | 1 | 0 | 全部通过 |

### 核心结果

**验证成功率**：所有资源均验证成功（Gateway 注入场景除外，见第 8 节）。非 HTML 文件全部通过，合约调用验证通过。

**验证耗时**：约为下载耗时的 0.4x ~ 6.1x。其中 EthStorage 文件验证通常低于下载耗时（0.4x - 0.7x），瓶颈在本地 KZG 计算；Ethereum 状态验证高于下载耗时（~6x），因 Colibri 需要获取状态证明，但绝对时间在可接受范围（~8s）。Prover 缓存命中后验证耗时进一步下降。

**浏览器可行性**：验证总耗时与下载总耗时处于同一量级（1.09x），已具备浏览器插件集成条件。
