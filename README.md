...
**验证耗时**：EthStorage 文件验证均低于下载耗时（0.32x ~ 0.66x），瓶颈在本地 KZG 计算。这类验证本质上也是合约状态调用（通过 Colibri 查询链上 Versioned Hash），但从取回数据到 Blob 编码生成 Hash，整个过程很快。NFT Render 合约验证耗时高于下载（2.90x），这并非"合约状态验证"方式本身的代价，之所以慢，是其合约自身特性导致：输出约 17KB（calldata 大数据），渲染结果由大量 JS 脚本碎片在合约内部拼接字符串而成，验证时 Colibri 需要跨合约调用获取这些碎片的证明和存储插槽数据，证明数据量大且密集传输。

**浏览器可行性**：验证总耗时（24.6s）仅为下载总耗时（51.2s）的 0.48x，已具备浏览器插件集成条件。
