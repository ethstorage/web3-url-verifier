...
**验证成功率**：所有资源均验证成功（Gateway 注入场景除外）。非 HTML 文件全部通过，合约调用验证通过。

**验证耗时**：EthStorage 文件验证均低于下载耗时（0.32x ~ 0.66x），瓶颈在本地 KZG 计算。NFT Render 合约验证耗时高于下载（2.90x），这并非"合约状态验证"方式本身的代价，而是其合约自身特性导致：输出约 17KB（calldata 大数据），且结果由大量 JS 脚本碎片在合约内部拼接字符串而成，验证时 Colibri 需要跨合约调用获取这些碎片的证明数据，数据量大且耗时。
