...
测试中使用的数据获取路径（w3link.io）会对部分 HTML 页面自动注入脚本，将 `web3://` 链接转换为 `https://` 链接：
...
值得注意的是，并非所有 HTML 页面都会受到影响。例如前文测试的 VBlog 首页验证正常，因为其页面结构未触发 Gateway 的注入逻辑；而其他案例中的 HTML 页面则被注入了额外内容。
